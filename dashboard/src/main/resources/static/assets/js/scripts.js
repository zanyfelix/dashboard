(function($) {
    ...
    //총 수집 문서 개수
	var list = $("span[name^='order']").length;
	
	var sum = 0;
	
	for(var i=1;i<list+1;i++) {
		
		var value = Number($("span[name='order"+i+"']").text());
	...
	//상태 변경
	var scope = $("th[scope='row']").length;
	
	for(var i=1;i<scope;i++) {
		
		var order = i*4-2;
		
		var categoryCnt = Number($("tr>td").eq(order).text());
		
		if(categoryCnt > 100 && categoryCnt < 300) {
			$("tr>td").eq(order-1).find("span").attr("class","badge badge-primary");
			$("tr>td").eq(order-1).find("span").text("보통");
		} else  if(categoryCnt > 300) {
			$("tr>td").eq(order-1).find("span").attr("class","badge badge-success");
			$("tr>td").eq(order-1).find("span").text("완료");
		}
	}
